...
evalStatement (env, pf) stmt = do
    case stmt of
        ...
        Qed -> case pf of
            Nothing -> throwError NoProof
            Just pf -> case runExcept $ qed pf of
                Left err -> throwError $ ProovingErr err
